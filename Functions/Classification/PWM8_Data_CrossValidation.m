...
%     plot_PWM_features(fPWM_train,fPWM_test,Np)
        ...
        [Mdl,Accuracy(num_fold),sensitivity(num_fold),specificity(num_fold),precision(num_fold),gmean(num_fold),f1score(num_fold),AUC(num_fold),ytrue,yfit,score]=Classify_Data(type_clf, fPWM_train, y_train, fPWM_test, y_test);
...
function fPWM= Generate_PWM8_features(Input_Sequence, PWM_P, PWM_N)
global Levels

    [Mer1_Seq_TS,name_Mer1] = Extract_Miers1(Input_Sequence,Levels);
    fPWM = Apply_General_PWM_feature_generator(Mer1_Seq_TS, PWM_P, PWM_N); 
